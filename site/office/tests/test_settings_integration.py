...
class TestSettingsIntegration:
    ...
    def test_setting_options_can_be_ordered(self):
        ...
        ordered_options = SettingOption.objects.filter(
            setting=canticle_setting,
            order__isnull=False
        ).order_by('order')
        ...
        if ordered_options.count() > 1:
            ...
            for i in range(len(option_list) - 1):
                assert option_list[i].order <= option_list[i + 1].order, \
    ...
    def test_all_settings_have_at_least_one_option(self):
        ...
        for setting in all_settings:
            ...
            assert options.count() > 0, \
                f"Setting '{setting.name}' should have at least one option"
    ...
    def test_setting_default_option_logic(self):
        ...
        settings_with_ordered_options = Setting.objects.filter(
            settingoption__order__isnull=False
        ).distinct()
        
        # Act & Assert
        for setting in settings_with_ordered_options:
            # Get the first option by order
            default_option = SettingOption.objects.filter(
                setting=setting,
                order__isnull=False
            ...
            all_ordered_options = SettingOption.objects.filter(
                setting=setting,
                order__isnull=False
            )
            min_order = min(opt.order for opt in all_ordered_options)
            assert default_option.order == min_order, \
