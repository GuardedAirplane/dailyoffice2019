...
def create_standard_office_days(db):
    ...
    to support date handling tests.
    """
...
class TestChurchYearTransitions:
    ...
    def test_eastertide_to_ordinary_time_transition(self):
        ...
        day_after_pentecost = date_class(2024, 5, 20)
        trinity_sunday = date_class(2024, 5, 26)
        day_after_trinity = date_class(2024, 5, 27)

        pentecost_cal = get_calendar_date(pentecost)
        day_after_pentecost_cal = get_calendar_date(day_after_pentecost)
        trinity_cal = get_calendar_date(trinity_sunday)
        after_trinity_cal = get_calendar_date(day_after_trinity)

        # Pentecost is the last day of Eastertide
        assert pentecost_cal.season.name == "Eastertide"

        # Season After Pentecost begins the day after Pentecost
        assert day_after_pentecost_cal.season.name == "Season After Pentecost"

        # Trinity Sunday itself
        assert "Trinity" in trinity_cal.primary.name
        assert trinity_cal.season.name == "Season After Pentecost"
