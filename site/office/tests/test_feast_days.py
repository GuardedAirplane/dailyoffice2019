...
from office.tests.factories import StandardOfficeDayFactory, ThirtyDayPsalterDayFactory
...
class TestFeastDayReadings:
    ...
    def setup_office_days(self, db):
        """Create StandardOfficeDay records for test dates."""
        # Create psalter days for all dates (1-31)
        for day in range(1, 32):
            ThirtyDayPsalterDayFactory.create(day=day)

        # Create StandardOfficeDay records for test dates
        test_dates = [
            (12, 25),  # Christmas
            (12, 10),  # Ordinary day in December
            (3, 31),  # Easter 2024 (movable)
            (1, 6),  # Epiphany
            (5, 9),  # Ascension 2024 (movable)
            (5, 19),  # Pentecost 2024 (movable)
        ]
...
class TestChristmasDayOffice:
    ...
    def setup_office_days(self, db):
        """Create StandardOfficeDay records for test dates."""
        # Create psalter days for December 25 and 26
        ThirtyDayPsalterDayFactory.create(day=25)
        ThirtyDayPsalterDayFactory.create(day=26)

        # Create StandardOfficeDay for Christmas and following day
        StandardOfficeDayFactory.create(month=12, day=25)
        StandardOfficeDayFactory.create(month=12, day=26)
    ...
    def test_christmas_evening_prayer(self):
        ...
        assert (
            "Christmas" in christmas_ep.date.primary_evening.name
            or "Nativity" in christmas_ep.date.primary_evening.name
        )
...
class TestEasterDayOffice:
    ...
    def setup_office_days(self, db):
        """Create StandardOfficeDay records for test dates."""
        # Create psalter days for March 31 and April 20
        ThirtyDayPsalterDayFactory.create(day=31)
        ThirtyDayPsalterDayFactory.create(day=20)

        # Create StandardOfficeDay for Easter dates
        StandardOfficeDayFactory.create(month=3, day=31)
        StandardOfficeDayFactory.create(month=4, day=20)
...
class TestOtherMajorFeasts:
    ...
    def setup_office_days(self, db):
        """Create StandardOfficeDay records for test dates."""
        # Create psalter days for all dates
        for day in [1, 6, 9, 14, 19, 26]:
            ThirtyDayPsalterDayFactory.create(day=day)

        # Create StandardOfficeDay for major feasts
        test_dates = [
            (1, 6),  # Epiphany
            (2, 14),  # Ash Wednesday 2024
            (3, 24),  # Palm Sunday 2024
            (3, 29),  # Good Friday 2024
            (3, 31),  # Easter 2024
            (4, 20),  # Easter 2025
            (5, 9),  # Ascension 2024
            (5, 19),  # Pentecost 2024
            (5, 26),  # Trinity Sunday 2024
            (11, 1),  # All Saints
        ]
        for month, day in test_dates:
            StandardOfficeDayFactory.create(month=month, day=day)
    ...
    def test_palm_sunday_office(self):
        ...
        assert "Palm Sunday" in palm_sunday.date.primary.name or palm_sunday.date.season.name == "Holy Week"
    ...
    def test_good_friday_office(self):
        ...
        assert "Good Friday" in good_friday.date.primary.name or good_friday.date.season.name == "Holy Week"
...
class TestFeastDayConsistency:
    ...
    def setup_office_days(self, db):
        """Create StandardOfficeDay records for test dates."""
        for day in [25, 26]:  # Christmas, St. Stephen
            ThirtyDayPsalterDayFactory.create(day=day)

        # Christmas and other test dates
        StandardOfficeDayFactory.create(month=12, day=25)
