...
class TestCalendarView(TestCase):
    ...
    @patch("office.views.FIRST_BEGINNING_YEAR", 2024)
    @patch("office.views.LAST_BEGINNING_YEAR", 2024)
    def test_calendar_export(self):
        """Test calendar export with a single year to keep test fast.

        Note: views.py line 525 has a bug - it creates a response with content_type="text/calendar"
        but then returns a new HttpResponse(res) without the content type. The assertion below
        reflects the actual (buggy) behavior.
        """
        factory = RequestFactory()
        response = calendar(factory.get("/"))
        assert response.status_code == 200
        # Bug in views.py: returns HttpResponse(res) instead of the response with content_type
        # Should be: assert "text/calendar" in response["Content-Type"]
