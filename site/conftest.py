...
def django_db_setup(django_db_setup, django_db_blocker):
    ...
    with django_db_blocker.unblock():
        ...
        # Get path relative to this conftest.py file
        conftest_dir = os.path.dirname(os.path.abspath(__file__))
        data_dump = os.path.join(conftest_dir, "dailyoffice_data_only.sql")
        ...
        if os.path.exists(data_dump):
            ...
            # Create a temporary SQL file that truncates tables, disables triggers, loads data
            import tempfile
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".sql", delete=False) as tmp:
                ...
                # Truncate Django's internal tables that conflict with the data dump
                # These tables are auto-populated by Django migrations but we need to
                # replace them with the data dump's versions for complete test data
                tmp.write("-- Truncate conflicting Django internal tables\n")
                tmp.write("TRUNCATE TABLE django_content_type CASCADE;\n")
                tmp.write("TRUNCATE TABLE auth_permission CASCADE;\n")
                tmp.write("TRUNCATE TABLE django_site CASCADE;\n")
                tmp.write("TRUNCATE TABLE django_migrations CASCADE;\n\n")

                # Ensure columns added after data dump have defaults for import
                # The data dump doesn't include these columns, so we set DB-level defaults
                tmp.write("-- Set database defaults for columns added after data dump was created\n")
                tmp.write("ALTER TABLE office_setting ALTER COLUMN setting_string_order SET DEFAULT 0;\n")
                tmp.write("ALTER TABLE office_settingoption ALTER COLUMN abbreviation SET DEFAULT 'A';\n\n")
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    # Reset sequences to avoid IntegrityError on new inserts
                    # This is critical because the data dump contains explicit IDs
                    # but doesn't update the sequences
                    ...
                    # Generate SQL to reset sequences
                    call_command("sqlsequencereset", *app_labels, stdout=output)
                    sql = output.getvalue()
                    ...
                    # Show warnings/notices from import (not full output)
                    if result.stderr:
                        stderr_lines = result.stderr.split("\n")
                        # Show first few non-INSERT lines
                        relevant_lines = [l for l in stderr_lines if l and "INSERT" not in l and "SET" not in l][:10]
                        if relevant_lines:
                            print("Import notices:")
                            for line in relevant_lines:
                                print(f"  {line}")

                    # Verify data was actually loaded by checking a key table
                    verify_result = subprocess.run(
                        [
                            ...
                            "-A",
                            "-c",
                            "SELECT COUNT(*) FROM office_setting;",
                        ],
                        env=env,
                        capture_output=True,
                        text=True,
                    ...
                    if verify_result.returncode == 0:
                        ...
                        print(f"✓ Verified: {count} settings loaded into test database")
                    else:
                        print("⚠ Could not verify data loading")
