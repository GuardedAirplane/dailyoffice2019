...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if acna_calendar:
            ...
            objects_to_export.extend(
                SanctoraleCommemoration.objects.filter(calendar=acna_calendar).select_related(
                    "rank", "calendar", "common"
                )
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully exported {len(objects_to_export)} objects to {output_file}")
        )
