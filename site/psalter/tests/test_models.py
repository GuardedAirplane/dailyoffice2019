...
class TestPsalmVerseModel(TestCase):
    ...
    def test_psalm_verse_unique_together(self):
        ...
        with self.assertRaises(Exception):
            PsalmVerse.objects.create(
                psalm=psalm_1,
                number=1,
                first_half="Duplicate",
                second_half="Duplicate"
            )
...
class TestPsalmTopicPsalmModel(TestCase):
    ...
    def test_psalm_topic_psalm_relationship(self):
        ...
        topic = PsalmTopic.objects.create(
            topic_name="Penitential",
            psalms="51",
            order=1
        )
        
        relationship = PsalmTopicPsalm.objects.create(
            psalm=psalm,
            psalm_topic=topic,
            order=1
        )
