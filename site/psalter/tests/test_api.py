...
def create_test_data(db):
    """
    Create minimal database entries required for Psalter API tests.

    Psalter API endpoints require psalm and psalter data.
    This fixture creates the minimum needed for tests to run.
    """
    from office.tests.factories import create_psalter_cycle, ThirtyDayPsalterDayFactory

    # Create complete psalter cycle (required for psalm retrieval)
...
class TestPsalmAPICrossStory:
    ...
    def test_psalm_in_morning_prayer_matches_psalm_api(self, client):
        ...
        # Get morning prayer using date within database range (2018-2021)
        mp_response = client.get("/api/v1/office/morning_prayer/2020-1-1")
